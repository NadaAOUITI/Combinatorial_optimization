...
class SAParameterTuner:
    ...
    def evaluate_combination(self, task):
        """
        Evaluate a single parameter combination.

        Args:
            task: A tuple containing (params, trial_num)
            
        Returns:
            Dictionary with results
        """
        params, trial_num = task  # Unpack the tuple
...
if __name__ == "__main__":
    ...
    for idx, instance in enumerate(instances):
        ...
        tuner = SAParameterTuner(instance, save_directory=save_dir)
        ...
        with open(f"{save_dir}/best_sa_parameters.json", "w") as f:
            json.dump(best_params, f, indent=4)

        print(f"✅ Saved best parameters for instance #{idx + 1} to {save_dir}/best_sa_parameters.json")
